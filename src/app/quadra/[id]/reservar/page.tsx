    ...
    <div className="min-h-screen bg-[#F9FAFB] flex flex-col">
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-10 text-center text-[#64748B]">
        <p className="text-lg font-semibold text-[#0D1F3C]" style={{ fontFamily: 'Syne, sans-serif' }}>
          Reservar Quadra #{params.id}
        </p>
        <p className="text-sm mt-1">Em construção — Tela 3</p>
        <Link href={`/quadra/${params.id}`} className="mt-4 inline-block text-sm text-[#2D5FA6] hover:underline">← Voltar</Link>
      </main>
      <Footer />
    </div>
  )
