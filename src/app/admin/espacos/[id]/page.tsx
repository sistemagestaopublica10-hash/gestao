...
export default function EspacoDetalhe() {
  ...
  const [avaliacoesItems, setAvaliacoesItems] = useState(
      ...
      {aba === 'detalhes' && (
        <div className="bg-white rounded-[14px] shadow-card p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <InfoItem
              label="Modalidades"
              value={espaco.modalidades.map(m => m.charAt(0).toUpperCase() + m.slice(1)).join(', ')}
            />
          ...
          {reservasDaQuadra.length > 0 && (
            <div>
              <h3 className="text-xs font-semibold text-[#64748B] uppercase tracking-wide mb-3 mt-2">
                Reservas recentes
              </h3>
                    ...
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                      ...
                      {notaMedia !== null && (
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-xs text-[#9CA3AF]">{notaMedia.toFixed(1)}</span>
                        </div>
                      )}
                    </div>
                    {avsDesteEspaco.length > 0 && (
                      <p className="text-[11px] text-[#9CA3AF] mt-1">
                        {avsDesteEspaco.length} avaliação(ões) neste espaço
                      </p>
                    )}
                  </div>
          ...
          <div className="bg-white rounded-[16px] shadow-xl w-full max-w-sm">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#F1F5F9]">
              ...
              <button
                ...
                className="p-1.5 rounded-full hover:bg-[#F1F5F9] text-[#9CA3AF] transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
